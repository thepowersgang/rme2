...
#define ENSURE_IN(State) do {\
	if( !State->Callbacks || !State->Callbacks->In ) { \
...
static inline int inB(tRME_State* State, uint16_t Port, uint8_t* Val) {
	ENSURE_IN(State);
	return State->Callbacks->In(State, Port, 1, Val);
}
static inline int inW(tRME_State* State, uint16_t Port, uint16_t* Val) {
	ENSURE_IN(State);
	return State->Callbacks->In(State, Port, 2, Val);
}
static inline int inD(tRME_State* State, uint16_t Port, uint32_t* Val) {
	ENSURE_IN(State);
	return State->Callbacks->In(State, Port, 4, Val);
}
#define ENSURE_OUT(State) do {\
	if( !State->Callbacks || !State->Callbacks->Out ) { \
